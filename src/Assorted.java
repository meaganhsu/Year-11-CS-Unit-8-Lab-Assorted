...
public class Assorted {
    ...
    public static int findSum(List<?> list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (list.get(i) instanceof String) sum += Integer.parseInt((String) list.get(i));
            else sum += (int) list.get(i);
        }
        return sum;
    }
    ...
    public static List<Integer> filterStrings(List list) {
        ...
        for (int i = 0; i < list.size(); i++) {
            if (!(list.get(i) instanceof String)) x.add((int) list.get(i));
        }
        return x;
    }
    ...
    public static List<Integer> filterNTimes(List<Integer> list, int n) {
        ...
        for (int i : list) if (!uni.contains(i)) uni.add(i);

        for (int h : uni) {
            n2 = n;
            for (int i = 0; i < list.size(); i++) {
                if (n2 > 0 && h == list.get(i)) n2--;
                else if (n2 <= 0 && h == list.get(i)) {
                    list.remove(i);
                    i--;
                }
            }
    ...
    public static List<String> wildWest(List<String> directions) {
        ArrayList<String> x = (ArrayList<String>) directions;

        for (int i = 1; i < x.size(); i++) {
            String one = x.get(i);
            String two = x.get(i-1);
        ...
        for (String i : directions) {
            if (i.equals("NORTH")) ns++;
            else if (i.equals("SOUTH")) ns--;
            else if (i.equals("EAST")) ew++;
            else if (i.equals("WEST")) ew--;
        }

        if (ns > 0) {
            for (int i = 0; i < ns; i++) x.add("NORTH");
        } else if (ns < 0) {
            for (int i = 0; i > ns; i--) x.add("SOUTH");
        }
        if (ew > 0) {
            for (int i = 0; i < ew; i++) x.add("EAST");
        } else if (ew < 0) {
            for (int i = 0; i > ew; i--) x.add("WEST");
        }
    ...
    public static boolean check(String x, String y) {
        ...
        else if (x.equals("EAST") && y.equals("WEST")) return true;
        else if (x.equals("WEST") && y.equals("EAST")) return true;
    ...
    public static int queueTime(List<Integer> queue, int tillsOpen) {
        ...
        for (int i = 0; i < tillsOpen; i++) q.add(i, 0);

        for (int i = 0; i < queue.size(); i++) {
            q.set(0, queue.get(i) + q.get(0));
            Collections.sort(q);
        }
